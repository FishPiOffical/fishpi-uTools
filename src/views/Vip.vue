    ...
    <div class="vip-page">
            ...
            <div class="hero-text">
                ...
                <div class="hero-cards">
                    <article v-for="tier in vipTiers" :key="tier.id" class="hero-card">
                        ...
                        <button class="btn" :class="{ disabled: currentUserVipInfo&&currentUserVipInfo.state !== 0 }"
                            @click="purchaseTier(tier)">
                            {{ currentUserVipInfo && currentUserVipInfo.state !== 0 ? "已是VIP" : "立即开通" }}
                        </button>
                    </article>
                </div>
            </div>
        ...
        <section class="vip-status-card" v-if="currentUserVipInfo && currentUserVipInfo.state !== 0">
            <div class="status-text">
                <p>欢迎 <span class="vip-level">{{ vipProfile.plan }}</span> 用户</p>
                <p>
                    会员到期时间：
            ...
            <div class="status-actions">
                ...
                <div class="nickname-preview" :class="[`${previewEffect}`, { bold: isBold, underline: hasUnderline }]"
                ...
                <div class="effect-grid">
                    ...
                    <div style="display: flex; flex-wrap: wrap; gap: 8px; margin-top: 8px;">
                        <button v-for="effect in nicknameEffects" :key="effect.key" class="effect-chip"
                            :class="{ active: previewEffect === effect.key, [effect.key]: true }"
...
<script setup>
    ...
    const parseBenefits = (benefitsStr) => {
        ...
        try {
            ...
            if (benefits.color) {
                if (benefits.color === "red" || benefits.color.startsWith("#")) {
                    features.push("昵称颜色");
                } else {
                    features.push("昵称渐变");
                }
    ...
    const vipTiers = computed(() => {
        const monthlyTiers = vipListData.value.filter(item => item.durationType === "月卡");
        const levelMap = {
    ...
    const priceTiers = computed(() => {
        ...
        tiersMap.forEach(tier => {
            const monthlyItem = vipListData.value.find(
                item => parseLvCode(item.lvCode).level === tier.id && item.durationType === "月卡"
            );
            const yearlyItem = vipListData.value.find(
                item => parseLvCode(item.lvCode).level === tier.id && item.durationType === "年卡"
            );
    ...
    const saveConfig = async () => {
        try {
            const config = {
                ...
                color: previewEffect.value === COLOR_EFFECT ? nicknameColor.value : previewEffect.value,
            };
            ...
            if (currentUserVipInfo.value?.configJson) {
                try {
                    ...
                    if (existingConfig.metal !== undefined) config.metal = existingConfig.metal;
                    if (existingConfig.jointVip !== undefined) config.jointVip = existingConfig.jointVip;
                    if (existingConfig.autoCheckin !== undefined) config.autoCheckin = existingConfig.autoCheckin;
                } catch (e) {
    ...
    const purchaseTier = async (tier) => {
        ...
        ElMessageBox.confirm(
            `确定要开通 ${tier.name} ${billingPeriod.value === "monthly" ? "月卡" : "年卡"} 吗？此操作不可恢复。`,
            "确认开通",
            {
            ...
            try {
                ...
                const durationType = billingPeriod.value === "monthly" ? "月卡" : "年卡";
                const targetItem = vipListData.value.find(
                    item => parseLvCode(item.lvCode).level === tier.id && item.durationType === durationType
                );
        ...
        try {
            ...
            if (res.code == 0) {
                if (res.data && res.data.state !== 0) {
                    ...
                    const tierName = vipListData.value.find(
                        item => item.lvCode === res.data.lvCode
                    )?.lvName || level;
